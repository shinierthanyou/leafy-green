...
plot(modA2)
summary(modA2)
cor.test(F1$Height, F1$Anth5, method= "spearman")
...
```{r}
summary(modA22)$r.squared
```

+ ANOVA Anthocyanin Low vs Mid Heights

```{r}
modA23 = lm(HL$Anth ~ HM$Anth)
anova(modA23)
plot(modA23)
...
```{r}
summary(modA23)$r.squared
```

+ ANOVA Anthocyanin High vs Low Heights

```{r}
modA24 = lm(HH$Anth ~ HL$Anth)
anova(modA24)
plot(modA24)
...
```{r}
summary(modC22)$r.squared
```

+ ANOVA Chlorophyll Low vs Mid Heights

```{r}
modC23 = lm(HL$Chl ~ HM$Chl)
anova(modC23)
plot(modC23)
...
```{r}
summary(modC23)$r.squared
```

+ ANOVA Chlorophyll High vs Low Heights

```{r}
modC24 = lm(HH$Chl ~ HL$Chl)
anova(modC24)
plot(modC24)
...
```{r}
summary(modF22)$r.squared
```

+ ANOVA Flavonol Low vs Mid Heights

```{r}
modF23 = lm(HL$Flav ~ HM$Flav)
anova(modF23)
plot(modF23)
```
...
```{r}
summary(modF23)$r.squared
```

+ ANOVA Flavonol High vs Low Heights

```{r}
modF24 = lm(HH$Flav ~ HL$Flav)
anova(modF24)
plot(modF24)
```
...
summary(mod183)
plot(F1$H185, F1$Row)
F1$Row2 = as.character(F1$Row)
F1$Row2 = as.numeric(F1$Row2)
class(F1$Row2)
cor.test(F1$Anth5,F1$Row2, method= "spearman")
```
...
HDP1 = ggplot(F1) + aes(x = Row, y = HD5) + geom_boxplot(fill = "dodgerblue1") + ylab("Height)") + xlab("Row") + ggtitle("Height Difference Between Years by Row") +theme_bw()+theme(plot.title = element_text(hjust = 0.5))+geom_hline(yintercept = 0, colour = "red");HDP1

HDP12 <-ggplot(R) +aes(x = Row, y = HDR) + geom_point() +geom_hline(yintercept = mean(F1$HD5, na.rm = T), colour = "dodgerblue1")+ ylab("Height") + xlab("Row") + ggtitle("Height Difference 2018-19")+theme_bw()+theme(plot.title = element_text(hjust = 0.5));HDP12
```
